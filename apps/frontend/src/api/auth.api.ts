...
export const AuthAPI = {
  ...
  sendOtp: async (target: string, type: 'EMAIL' | 'PHONE') => {
    const response = await apiClient.post('/otp/send', { target, type });
    return response.data;
  },

  verifyOtp: async (target: string, type: 'EMAIL' | 'PHONE', otp: string) => {
    const response = await apiClient.post('/otp/verify', { target, type, otp });
    return response.data;
  },

  forgotPassword: async (email: string) => {
    // This will trigger the Redis OTP for the 'EMAIL' type on the backend
    const response = await apiClient.post('/otp/send', { target: email, type: 'EMAIL' });
    return response.data;
  },
