...
export default function ModuleApp() {
  ...
  // Helper to calculate the exact opposite color for high contrast
  const getInvertedSvgColor = (hex: string) => {
    if (!hex) return '%23cbd5e1'; // default slate fallback
    const cleanHex = hex.replace('#', '');
    if (cleanHex.length !== 6) return '%23cbd5e1'; 
    
    // Invert RGB values and convert back to hex
    const r = (255 - parseInt(cleanHex.slice(0, 2), 16)).toString(16).padStart(2, '0');
    const g = (255 - parseInt(cleanHex.slice(2, 4), 16)).toString(16).padStart(2, '0');
    const b = (255 - parseInt(cleanHex.slice(4, 6), 16)).toString(16).padStart(2, '0');
    
    return `%23${r}${g}${b}`; // %23 is the URL-encoded '#'
  };
  ...
  return (
    <div 
      ...
      style={{
        backgroundColor: 'color-mix(in srgb, var(--theme-500) 20%, #f8fafc)',
        backgroundImage: `
          linear-gradient(135deg, color-mix(in srgb, var(--theme-500) 6%, transparent) 0%, transparent 100%),
          url("data:image/svg+xml,%3Csvg width='24' height='24' viewBox='0 0 24 24' xmlns='http://www.w3.org/2000/svg'%3E%3Ccircle cx='2' cy='2' r='1.5' fill='${svgDotColor}' fill-opacity='0.25'/%3E%3C/svg%3E")
        `,
